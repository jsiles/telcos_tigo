...
//		     		echo $sTotalActivo . "--".$sTotalPasivo;
    ...
    $periodo = db_fill_array("select per_periodo, per_periodo from tb_periodos where per_jue_id=$jue_id and per_estado='A'");
									  		...
									  		if ($key==$per_id)
									  		 {                                                       
												...
												if (($iCantidad==0)||(($iCantidad1!=0)&&($key==$periodoInicial))) {
													if ($accion=='') $ele_monto = "&nbsp;";
													else $ele_montoDisplay = "<input type=\"text\" name=\"ele_elemento[$id]\" value =\"$ele_monto\" size=\"4\" maxlength=\"6\" style=\"text-align:right;font-size:9;color:#000066;border:thin;background-color:#F4F4F4\">";
												}
									  		     else
									  		     {
...
//            {   
            ...
            for ($key=$sSelPeriodo;$key<=$sSelPeriodo;$key++)
            {
                ...
                {
                  $keytemp = $key-1;
                  $clvtemp = get_db_value("select max(tri_id) from tb_trimestres where tri_sw='A'");//5;7//últino trimestre
...
//////////////////////////////////// VALORES INICIALES/////////////////////////////////77
                $periodo_inicio = get_db_value("select jue_periodoInicial from tb_juegos where jue_id=$jue_id");
				//si el primer periodo coincide se saca el valor inicial de la tabla inicio
                ...
                else 
                {
					...
					//echo "select cal_monto from tb_calculos where cal_usu_id=$user_id and cal_pro_id=$dat_producto and cal_mer_id=$dat_mercado and cal_cli_id=$cli_id and cal_periodo=$keytemp and cal_trimestre=$clvtemp and cal_ite_id=$ite_id ";
                    $valini=$valorPerant;
                                ...
                                //echo 'item '. $ope_ite_id.' operacion '.$operacion.' valor '.$valor.' atributo '. $atr_id. ' trim '. $trimestre. '<br>';         
                                    ...
                                    if ($atr_id!='')
                                        ...
                                        {   
                                            ...
                                            {               
                                                            $keytemp = $key-1;
                                                            $clvtemp = get_db_value("select max(tri_id) from tb_trimestres where tri_sw='A'");//5;6//último trimestre
