...
function alta()
{
    ...
    if ($nombreJuego && $juego)
    {
        ...
        ****************************************/
        ...
        $sSQL = "select mer_nombre from tb_mercados where mer_jue_id=$juego";
        $db->query($sSQL);
        $nex_record = $db->next_record();
        while ($nex_record)
        {
            $nombre_mercado = $db->f("mer_nombre");
            $sQuery = "insert into tb_mercados values (null, $max_juego,".tosql($nombre_mercado, "Text").",'A')";
            $db2->query($sQuery);
            $nex_record = $db->next_record();
        }
        ...
        ****************************************/
        ...
        $sSQL = "select pro_nombre from tb_productos where pro_jue_id=$juego";
        $db->query($sSQL);
        $nex_record = $db->next_record();
        while ($nex_record)
        {
            $nombre_producto = $db->f("pro_nombre");
            $sQuery = "insert into tb_productos values (null, $max_juego,".tosql($nombre_producto, "Text").",'A')";
            $db2->query($sQuery);
            $nex_record = $db->next_record();
        }        
        ...
        ****************************************/
        ...
        $sSQL = "insert into tb_periodos (select null, $max_juego, per_periodo, per_estado, per_inv_estado, per_tiempo, per_datetime from tb_periodos where per_jue_id=$juego order by per_id)";
        $db->query($sSQL);      
