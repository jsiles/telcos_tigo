    ...
    while($next_record)
    {
        ...
        $value="";
            ...
            for ($key=$sSelPeriodo;$key<=$sSelPeriodo;$key++)
            {
                ...
                $sValor=get_db_value("select sum(vai_monto) from th_valoresiniciales where vai_periodo=$key and vai_cli_id=$cli_id and vai_atr_id=$clv and vai_jue_id=$jue_id");
                if ($sValor=='') $sValor=0;
                if ($atr_tipoValor=='P') 
                { 
                    //$sValor = $sValor * 100;
...
//            {   
            ...
            for ($key=$sSelPeriodo;$key<=$sSelPeriodo;$key++)
            {
                ...
                {
                    $valini = get_db_value("select sum(ini_monto) from th_inicio where ini_jue_id=$jue_id and ini_tic_id=$cli_id");
                    if ($valini=="") $valini=0;
                }
                else
                {
                  ...
                  $iCalculos = "select sum(val_cantidad) from th_valores where val_tri_id=".$clvtemp." and val_periodo=$keytemp and val_cli_id=$cli_id and val_usu_id=$user_id and val_sw='A' and val_pro_id!=66 and val_mer_id!=66 order by val_id";
                  //echo $iCalculos.'<br>';
...
//                $tpl->parse("Valores", true);
                ...
                {
                    $iValor = get_db_value("select sum(val_cantidad) from th_valores where val_usu_id=$user_id  and val_cli_id=$cli_id and val_tri_id=$clv and val_periodo=$key and val_pro_id!=66 and val_mer_id!=66");
...
//////////////////////////////////// VALORES INICIALES/////////////////////////////////77
                ...
                else 
                {
                    $keytemp= $key-1;
                    $clvtemp = get_db_value("select max(tri_id) from tb_trimestres where tri_sw='A'");//5;último trimestre
                    $vSQL ="select sum(cal_monto) from tb_calculos where cal_usu_id=$user_id and cal_cli_id=$cli_id and cal_periodo=$keytemp and cal_trimestre=$clvtemp and cal_ite_id=$ite_id ";
                    $valorPerant = get_db_value($vSQL);
                   // echo '<br>'.$vSQL;
                    $valini=$valorPerant;
              ...
              foreach($sSQL2 as $clv => $value2)
                {
                        ...
                        $iValor = get_db_value("select sum(cal_monto) from tb_calculos where cal_trimestre=$clv and cal_usu_id=$user_id and cal_periodo=$key and cal_cli_id=$cli_id and cal_ite_id=$ite_id ");
