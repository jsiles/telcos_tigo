...
//            {   
            ...
            for ($key=$sSelPeriodo;$key<=$sSelPeriodo;$key++)
            {
                ...
                {
                  $keytemp = $key-1;
                  $clvtemp = get_db_value("select max(tri_id) from tb_trimestres where tri_sw='A'");//5;último trimestre
              ...
              foreach($sSQL2 as $clv => $value2)
                {
                        ...
                        $iValor = get_db_value("select sum(cal_monto) from tb_calculos where cal_trimestre=$clv and cal_usu_id=$user_id and cal_periodo=$key and cal_mer_id =$dat_mercado and cal_cli_id=$cli_id and cal_ite_id=$ite_id ");
                    if ($clv) {
                        //echo $ite_millones;
