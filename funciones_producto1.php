    ...
    while($next_record)
    {
        ...
        $value="";
            ...
            for ($key=$sSelPeriodo;$key<=$sSelPeriodo;$key++)
            {
                ...
                $sValor=get_db_value("select sum(vai_monto) from th_valoresiniciales where vai_periodo=$key and vai_cli_id=$cli_id and vai_mer_id=$dat_mercado and vai_atr_id=$clv and vai_jue_id=$jue_id");
                if ($sValor=='') $sValor=0;
                if ($atr_tipoValor=='P') 
                { 
                    $sValor = $sValor * 100;
...
//            {   
            ...
            for ($key=$sSelPeriodo;$key<=$sSelPeriodo;$key++)
            {
                ...
                {
                    $valini = get_db_value("select sum(ini_monto) from th_inicio where ini_jue_id=$jue_id and ini_tic_id=$cli_id and ini_mer_id=$dat_mercado");
                    if ($valini=="") $valini=0;
                }
                else
                {
                  $keytemp = $key-1;
                  $clvtemp = 5;
                  $iCalculos = "select sum(val_cantidad) from th_valores where val_tri_id=".$clvtemp." and val_periodo=$keytemp and val_cli_id=$cli_id and val_mer_id=$dat_mercado and val_usu_id=$user_id and val_sw='A' order by val_id"; 
...
//                $tpl->parse("Valores", true);
                ...
                {
                    $iValor = get_db_value("select sum(val_cantidad) from th_valores where val_usu_id=$user_id and val_mer_id=$dat_mercado and val_cli_id=$cli_id and val_tri_id=$clv and val_periodo=$key ");
...
//////////////////////////////////// VALORES INICIALES/////////////////////////////////77
                ...
                if ($key==$periodo_inicio)
                {
                    $valini = get_db_value("select sum(ini_monto) from th_inicio where ini_jue_id=$jue_id and ini_mer_id=$dat_mercado and ini_tic_id=$cli_id");
                    $vValini+= $valini;
                ...
                else 
                {
                    $keytemp= $key-1;
                    $clvtemp = 5;
                    $valorPerant = get_db_value("select sum(cal_monto) from tb_calculos where cal_usu_id=$user_id and cal_mer_id=$dat_mercado and cal_cli_id=$cli_id and cal_periodo=$keytemp and cal_trimestre=$clvtemp and cal_ite_id=$ite_id");
//                    echo $valorPerant.'-';
                    $valini=$valorPerant;
              ...
              foreach($sSQL2 as $clv => $value2)
                {
                                    ...
                                    if ($atr_id!='')
                                        {
                                            ...
                                            $iCalculos = "select sum(vai_monto) from th_valoresiniciales where vai_periodo=$key and vai_cli_id=$cli_id and vai_mer_id=$dat_mercado and vai_atr_id=$atr_id and vai_jue_id=$jue_id and vai_sw='A' order by vai_id";
                                        ...
                                        {   
                                            ...
                                            if ($trimestre==0) {
                                                ...
                                                if ($qValida!=0)
                                                {
                                                $iCalculos = "select sum(val_cantidad) from th_valores where val_tri_id=$clv and val_periodo=$key and val_cli_id=$cli_id and val_mer_id=$dat_mercado and val_usu_id=$user_id and val_sw='A' order by val_id"; 
                                                $millones = 0;
                                                }
                                                else {
                                                    $iCalculos = "select sum(cal_monto) from tb_calculos where cal_trimestre=$clv and cal_periodo=$key and cal_cli_id=$cli_id and cal_mer_id=$dat_mercado and cal_usu_id=$user_id and cal_ite_id=$ope_ite_id order by cal_id"; 
                                                    $millones = 1;
                                                }
                                                ...
                                                {   
                                                   // $vTotalAtr[$clv] =0; ////1
                                                    $keytemp = $key-1;
                                                            $keytemp = $key;
                                                            $clvtemp = $clv-1;
                                                    $iCalculos = "select sum(cal_monto) from `tb_calculos` where cal_trimestre=$clvtemp and cal_periodo=$keytemp and cal_cli_id=$cli_id and cal_mer_id=$dat_mercado and cal_usu_id=$user_id and cal_ite_id=$ope_ite_id order by cal_id ";
                                            ...
                                            {               
                                                            $keytemp = $key-1;
                                                            $clvtemp = 5;
                                                            $iCalculos = "select sum(cal_monto) from tb_calculos where cal_trimestre=".$clvtemp." and cal_periodo=$keytemp and cal_cli_id=$cli_id and cal_mer_id=$dat_mercado and cal_usu_id=$user_id and cal_ite_id=$ite_id ";                                                 
                                                            echo $iCalculos."<br>";         
                                            ...
                                            else if (($clv==2)&&($key==$periodo_inicio))                                    
                                              {
                                                 ...
                                                 $iCalculos = "select sum(ini_monto) from th_inicio where ini_jue_id=$jue_id and ini_tic_id=$cli_id and ini_mer_id=$dat_mercado";              
                    ...
                    if ($clv) {
                        ...
                        if ($ite_millones==0)
                            {  echo '<font color=red>'.$iValor.'</font><br>';
