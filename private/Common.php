...
//CCLoginUser @0-1CC60899
function CCLoginUser($login, $password)
{
    $db = new clsDBsiges();
    $SQL = "SELECT usu_id, usu_nivel FROM tb_usuarios WHERE usu_login=" . $db->ToSQL($login, ccsText) . " AND usu_password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if($Result)
    {
        CCSetSession("UserID", $db->f("usu_id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("usu_nivel"));
    }
    $db->close();
    return $Result;
}
